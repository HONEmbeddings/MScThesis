...
class ABCSymmetricEmbedding(ABCEmbedding):
    def __init__(self, gen: ABCHigherOrderPathGenerator, dimension: int, nodes: Iterator[Any]):
        super().__init__(gen, dimension, symmetric=True)
        self._nodes = { n:i for i,n in enumerate(nodes) }
        self._embedding = np.zeros(shape=(len(self._nodes), dimension))
        self.key2str = repr 

    def __getitem__(self, key) -> pd.Series:
        idx = self._nodes[key]
        return pd.Series(self._embedding[idx,:], name=self.key2str(key))
    ...
    def nodes_str(self) -> List[str]:
        return list(map(self.key2str, self.nodes))

    @property
    def embedding(self) -> pd.DataFrame:
        return pd.DataFrame(self._embedding, index=self.nodes_str)
    ...
    def _decode_raw(self, node: Any, step = None) -> np.array:
        ...
        """
        return np.exp(self._embedding[self._nodes[node]] @ self._embedding.T)
        ...
        **kwargs are passed to _decode_raw(node)
        ...
        if use_neighborhood:
            ...
            for _,next_node,_ in self._gen.transition_probs(start=(node,)):
                i = self._nodes[next_node]
                data_n[i] = data[i]
            data = data_n
        if no_self_loops:
            data[self._nodes[node]] = 0
        if normalize:
            data = data / max(1e-16, data.sum())
        return pd.Series(data, index=self.nodes_str, name=self.key2str(node))
...
class ABCAsymmetricEmbedding(ABCEmbedding):
    def __init__(self, gen: ABCHigherOrderPathGenerator, dimension: int, source_paths: Iterator[Tuple[Any,...]], target_nodes: Iterator[Any]):
        super().__init__(gen, dimension, symmetric=False)
        self._source_paths = { n:i for i,n in enumerate(source_paths) }
        self._target_nodes = { n:i for i,n in enumerate(target_nodes) }
        self._source_embedding = np.zeros(shape=(len(self._source_paths), dimension))
        self._target_embedding = np.zeros(shape=(len(self._target_nodes), dimension))
        self.path2str = repr
        self.node2str = repr
    ...
    def __getitem__(self, key) -> pd.Series:
        ...
        """
        idx = self._target_nodes.get(key, None)
        if idx is not None:
            return pd.Series(self._target_embedding[idx,:], name=self.node2str(key))
        idx = self._source_paths.get(key, None)
        if idx is not None:
            return pd.Series(self._source_embedding[idx,:], name=self.path2str(key))
        else:
    ...
    def source_paths(self) -> Iterator[Tuple[Any,...]]:
        return self._source_paths.keys()
    
    @property
    def source_paths_str(self) -> List[str]:
        return list(map(self.path2str, self.source_paths))

    @property
    def target_nodes(self) -> Iterator[Any]:
        return self._target_nodes.keys()

    @property
    def target_nodes_str(self) -> List[str]:
        return list(map(self.node2str, self.target_nodes))

    @property
    def source_embedding(self) -> pd.DataFrame:
        return pd.DataFrame(self._source_embedding, index=self.source_paths_str)

    @property
    def target_embedding(self) -> pd.DataFrame:
        return pd.DataFrame(self._target_embedding, index=self.target_nodes_str)
    
    @property
    def dimension(self):
        "effective dimension of embedding"
        return self._source_embedding.shape[1]
    ...
    def _decode_raw(self, start: Tuple[Any,...], step = None) -> np.array:
        ...
        (The parameter step is not used.)
        """
        return np.exp(self._source_embedding[self._source_paths[start]] @ self._target_embedding.T)
    ...
    def decode(self, start: Tuple[Any,...], use_neighborhood: bool = False, no_self_loops: bool = False, normalize: bool = True, **kwargs) -> pd.Series:
        """Calculate probabilities from the embedding.
        The parameter start must contain a value from self.source_paths.
        ...
        **kwargs are passed to _decode_raw(start)
        ...
        if use_neighborhood:
            ...
            for _,next_node,_ in self._gen.transition_probs(start=start[-1:]):
                i = self._target_nodes[next_node]
                data_n[i] = data[i]
            data = data_n
        if no_self_loops:
            data[self._target_nodes[start[-1]]] = 0
        if normalize:
            data = data / max(1e-16, data.sum())
        return pd.Series(data, index=self.target_nodes_str, name=self.path2str(start))

    def decode_path(self, start: Tuple[Any,...], **kwargs) -> pd.Series:
        "Finds the matching source_path and returns decode()"
        if start in self._source_paths:
...
class Generic_SkipGram_Embedding(ABCSymmetricEmbedding):
    ...
    def __init__(self, gen: ABCHigherOrderPathGenerator, emb_path : str, id: str = None, parse_node=lambda x:x, binary: bool=False, config=dict()):
        ...
        super().__init__(gen, dimension=int(dims[1]), nodes=gen.target_nodes)
        self._id = os.path.splitext(os.path.split(emb_path)[-1])[0] if id is None else id
        self._emb_path = emb_path
        #self._binary = binary
        assert self._embedding.shape == emb.values.shape, f'shapes do not match: {self._embedding.shape}!={emb.values.shape}'
...
class HON_DeepWalk_Embedding(ABCSymmetricEmbedding):
    ...
    def __init__(self, gen: ABCHigherOrderPathGenerator, dimension: int, reuse_walks: bool = False):
        super().__init__(gen, dimension, gen.target_nodes)
        self._id = 'DeepWalk'
    ...
    def train(self, num_walks:int=100, walk_length:int=80, window_size=10, num_iter:int=1, min_count:int=0,
            hs:bool = True, negative:int=5, workers:int=1, random_seed=None, replace_hash:bool=True, **kwargs):
        # reuse_walks allows for efficiently evaluating different params 
        # of Word2vec (e.g., negative, num_iter).
        # walks depend only on num_walks, walk_length, and random_seed.
        tc = self._training_config # config of previous training
        can_reuse_walks = tc['num_walks']==num_walks and tc['walk_length']==walk_length and tc['random_seed']==random_seed
        ...
        for v,iv in self._nodes.items():
            self._embedding[iv,:] = model.wv[str(v)]
...
class HON_Node2vec_Embedding(HON_DeepWalk_Embedding):
    ...
    def set_params(self, p: float = 1, q: float = 1):
        self.gen.set_params(p, q) # do not call this directly
        self._id = 'Node2vec(p={0}, q={1})'.format(p,q)
        self._walks = None
        #self._embedding = np.zeros(self._embedding.shape) # reset embedding
...
class HONEM_Embedding(ABCAsymmetricEmbedding):
    ...
    def neighborhood_matrix(self, order: int=1, sort=True) -> pd.DataFrame:
        ...
        if sort:
            res = res.loc[self.source_paths_str]
            res = res[self.target_nodes_str]
        return res

    def __init__(self, gen: HigherOrderPathGenerator, dimension: int):
        super().__init__(gen, dimension, gen.source_paths_len1, gen.target_nodes)
        self._id = 'HONEM'
    ...
    def train(self):
        ...
        # sort
        neighborhood = neighborhood.loc[self.source_paths_str]
        neighborhood = neighborhood[self.target_nodes_str]
    ...
    def _decode_raw(self, start: Tuple[Any,...], step = None) -> np.array:
        ...
        return self._source_embedding[self._source_paths[start]] @ self._target_embedding.T
...
class HON_NetMF_Embedding(ABCAsymmetricEmbedding):
    ...
    def __init__(self, gen: ABCHigherOrderPathGenerator, dimension: int, pairwise=False):
        source_paths = gen.source_paths_len1 if pairwise else gen.source_paths
        super().__init__(gen, dimension, source_paths, gen.target_nodes)
    ...
    def train(self, window_size:int, negative:int=1, optimized:bool=True):
        assert window_size > 0, 'window_size must be a positive integer'
        self._training_config = dict(window_size=window_size, negative=negative, optimized=optimized)
        if window_size != self._window_size:
            # calculate pointwise mutual information (PMI) - this takes some time
            PMI = np.zeros(shape=(len(self.source_paths), len(self.target_nodes)))
            idx = { v:i for i,v in enumerate(self.target_nodes) }
            ...
            if optimized:
                ...
                max_len = self._gen.max_rule_key_length
                for iu,u in enumerate(self.source_paths):
    ...
    def PMI(self) -> pd.DataFrame:
        return pd.DataFrame(self._PMI, index=self.source_paths_str, columns=self.target_nodes_str)
...
class HON_GraRep_Embedding(ABCAsymmetricEmbedding):
    ...
    def __init__(self, gen: ABCHigherOrderPathGenerator, dimension: int, num_steps: int = 4, pairwise:bool=True, neg_stationary:bool=False):
        ...
        super().__init__(gen, self._dimension_per_step * num_steps, source_paths, gen.target_nodes)
    ...
    def train(self, negative: int = 1, normalize: bool = False):
        ...
        self._training_config = dict(negative=negative, normalize=normalize)
        if self._PMI is None:
            idx = { v:i for i,v in enumerate(self.target_nodes) }
            PMI = np.zeros(shape=(len(self.source_paths), len(self.target_nodes), self._num_steps))
            max_len = self._gen.max_rule_key_length
            for iu,u in enumerate(self.source_paths):
        ...
        for step0 in range(self._num_steps):
            ...
            self._source_embedding[:, indices] = source_embedding
            self._target_embedding[:, indices] = target_embedding

    def PMI(self, step: int = 1) -> pd.DataFrame:
        return pd.DataFrame(self._PMI[:, :, step - 1], index=self.source_paths_str, columns=self.target_nodes_str)
    ...
    def _decode_raw(self, start: Tuple[Any,...], step: int = 1) -> pd.Series:
        ...
        return np.exp(self._source_embedding[self._source_paths[start], indices] @ self._target_embedding[:,indices].T)
...
class HON_Transition_Hierarchical_Embedding(ABCAsymmetricEmbedding):
    ...
    def __init__(self, gen: ABCHigherOrderPathGenerator, dimension: int=128, 
        ...
        super().__init__(gen, dimension, source_paths, target_nodes)
    ...
    def clone(self):
        ...
        res._source_embedding = self._source_embedding.copy()
        res._target_embedding = self._target_embedding.copy()
    ...
    def reset(self, seed=None):
        ...
        self._total_steps = 0
        self._source_embedding = self._rng.normal(0, 1/dimension, (len(self._source_paths),dimension))
        self._target_embedding = self._rng.normal(0, 1/dimension, (len(self._target_nodes),dimension))
    ...
    def _calc_objective(self) -> Tuple[float,float,float]:
        ...
        if self._neg_stationary: # stationary distribution
            ...
            n_target = len(self._target_nodes)
            neg_probs = { next_node:1/n_target for next_node in self._target_nodes }
        for u,iu in self._source_paths.items(): # uniform sampling
            probs = { next_node:prob for _,next_node,prob in self._gen.transition_probs(u) }
            for v,iv in self._target_nodes.items():
                score = self._source_embedding[iu,:] @ self._target_embedding[iv,:]
        ...
        for u,pu in self._parent.items():
            iu = self._source_paths[u]
            ipu = self._source_paths[pu]
            loss_penalty += np.linalg.norm(self._source_embedding[iu,:] - self._source_embedding[ipu,:])**2
    ...
    def _update(self, iu, iv, label, learning_rate):
        "SGD update"
        eu = self._source_embedding[iu,:]
        ev = self._target_embedding[iv,:]
        score = eu @ ev
        grad = learning_rate * (self.sigmoid(score) - label)
        self._source_embedding[iu,:] -= grad * ev
        self._target_embedding[iv,:] -= grad * eu
    ...
    def _update_hierarchy(self, u, iu, learning_rate, penalty, max_start_len: int):
        ...
        if u in self._children:
            if len(u) == max_start_len:
                return
            ichildren = list(self._source_paths[cu] for cu in self._children[u])
            ec = self._source_embedding[ichildren,:].mean(axis=0) # average embedding for all children
            self._source_embedding[iu,:] += factor * (ec - self._source_embedding[iu,:])
        
        pu = self._parent.get(u, None)
        if not pu is None:
            ipu = self._source_paths[pu]
            self._source_embedding[iu,:] += factor * (self._source_embedding[ipu,:] - self._source_embedding[iu,:])
    ...
    def train(self, steps: int=1, negative: int=1, penalty: float=0, 
        ...
        if max_start_len is None:
            source_paths_s = list((u,iu) for u,iu in self._source_paths.items())
            max_start_len_eff = max(len(key) for key in source_paths_s)
        else:
            source_paths_s = list((u,iu) for u,iu in self._source_paths.items() if len(u) <= max_start_len)
        ...
        def copy_long_key_embeddings():
            if max_start_len is not None:
                ...
                for u,iu in self._source_paths.items():
                    if len(u) > max_start_len:
                        pu = u[(-max_start_len_eff):] # FIXME: hardcoded assumption about hode_hierarchy!
                        ipu = self._source_paths[pu]
                        self._source_embedding[iu,:] = self._source_embedding[ipu,:]

        n_to = len(self._target_nodes)
