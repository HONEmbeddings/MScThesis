...
class EmbeddingView(object):
    ...
    def __init__(self, emb : ABCEmbedding, use_source: bool = True):
        ...
        if isinstance(emb, ABCSymmetricEmbedding):
            self._keys = list(emb.nodes) 
            self._keys_str = emb.nodes_str
            self.key2str = emb.key2str
            use_source = False
        elif use_source:
            self._keys = list(emb.source_paths)
            self._keys_str = emb.source_paths_str
            self.key2str = emb.path2str
        else:
            self._keys = list(emb.target_nodes)
            self._keys_str = emb.target_nodes_str
            self.key2str = emb.node2str
        self.use_source = use_source # True if key is a tuple of nodes
        if use_source:
            for name, data in emb._gen._source_path_metadata.items():
                self.add_metadata(name, data)
        else:
            for name, data in emb._gen._target_node_metadata.items():
                self.add_metadata(name, data)
    ...
    def node2key(self, node:Any):
        ...
        #return self._emb.node2key(node, self.use_source) # todo: remove ABCEmbedding.node2key?
    ...
    def embedding(self) -> pd.DataFrame:
        ...
        if isinstance(self._emb, ABCSymmetricEmbedding):
            return self._emb.embedding
        elif self.use_source:
            return self._emb.source_embedding
        else:
            return self._emb.target_embedding
    ...
    def key_embedding_diff(self, key_pairs) -> np.ndarray:
        ...
        for start,end in key_pairs:
            e_start = self._emb[start] # = self.embedding.loc[key2str(start)]
            e_end = self._emb[end] # = self.embedding.loc[key2str(end)]
            tmp[i,:] = e_end - e_start
            i+=1
        return tmp
...
## Usage:
# from Visualizations import Lattice2D_EmbeddingView, EmbeddingView, Visualization
...
# ev = Lattice2D_EmbeddingView(emb)
